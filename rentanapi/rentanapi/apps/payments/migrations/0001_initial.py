# Generated by Django 2.2 on 2020-01-17 13:52

from django.conf import settings
from django.db import migrations, models
import django.db.models.deletion


class Migration(migrations.Migration):

    initial = True

    dependencies = [
        ('article', '0004_special_follow_count'),
        migrations.swappable_dependency(settings.AUTH_USER_MODEL),
    ]

    operations = [
        migrations.CreateModel(
            name='Reward',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('is_show', models.BooleanField(default=False, verbose_name='是否显示')),
                ('orders', models.IntegerField(default=1, verbose_name='排序')),
                ('is_deleted', models.BooleanField(default=False, verbose_name='是否删除')),
                ('created_time', models.DateTimeField(auto_now_add=True, verbose_name='创建时间')),
                ('updated_time', models.DateTimeField(auto_now=True, verbose_name='更新时间')),
                ('money', models.DecimalField(decimal_places=2, max_digits=6, verbose_name='打赏金额')),
                ('status', models.BooleanField(default=False, verbose_name='是否支付')),
                ('trade_no', models.CharField(blank=True, max_length=255, null=True, verbose_name='流水账号')),
                ('out_trade_no', models.CharField(blank=True, max_length=255, null=True, verbose_name='支付宝流水账号')),
                ('reward_type', models.IntegerField(choices=[(1, '支付宝'), (2, '余额')], default=1, verbose_name='支付方式')),
                ('message', models.TextField(blank=True, null=True, verbose_name='留言')),
                ('article', models.ForeignKey(on_delete=django.db.models.deletion.DO_NOTHING, to='article.Article', verbose_name='文章')),
                ('user', models.ForeignKey(on_delete=django.db.models.deletion.DO_NOTHING, to=settings.AUTH_USER_MODEL, verbose_name='用户')),
            ],
            options={
                'verbose_name': '打赏记录',
                'verbose_name_plural': '打赏记录',
                'db_table': 'rr_user_reward',
            },
        ),
    ]
